...
{  
    ...
    { 
	    ...
	    if (Keyboard::isKeyPressed(Keyboard::Up))	if (.onGround) { .dy=-0.27; .onGround=false;  sound.play();}
		 ...
		 {
			 if (enemy.life) {
			 if (.dy>0) { enemy.dx=0; Mario.dy=-0.2; enemy.life=false;}
			 else .sprite.setColor(Color::Red);
			 }
		 }


		 if (.rect.left>200) offsetX = Mario.rect.left-200;           //смещение
