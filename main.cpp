...
int main()
{
	...
	Texture tileSet;
	tileSet.loadFromFile("Player_Tileset.png");
	...
	Music music;
	music.openFromFile("Player_Theme.ogg");
	music.play();
	...
	while (window.isOpen())
	{
		...
		if (Keyboard::isKeyPressed(Keyboard::Up))	if (Player.onGround) { Player.dy = -0.27; Player.onGround = false;  sound.play(); }
